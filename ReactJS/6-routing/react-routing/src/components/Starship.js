...
const Starship = () => {
    ...
    useEffect(() => {
        fetch(`https://swapi.dev/api/starships/${params.starshipId}`)
            .then(res => res.json())
            .then(result => {
                setStarship(result);
            })
    ...
    }, [params.starshipId, navigate]);

    const nextProductHandler = () => {
        navigate(`/starships/${Number(params.starshipId) + 1}`, {replace: false});
    };

    return (
        <>
        <h2>Starships page</h2>
        <h3>Starship {params.starshipId} Specification</h3>
